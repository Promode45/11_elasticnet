...
Ridge selects variables and takes it really close to zero and lasso pushes it to zero 
...
library(tidyverse)
library(glmnet)
library(vip) #plots of variable importance 
...
First, let's specify:  
  ...
  - which **engine** we want to use  - packages used to train the model
...
enet_grid_result %>%
  ...
  labs(y = "RMSE")
```
collect_metrics gives us the average values across v folds for different hyperparameter combinations 
